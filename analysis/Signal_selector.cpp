...
int main(int argc, char **argv){
	...
	TTree SkimTree("Tree", "A skimmed tree containing six selected nBToKEE candidates");
		...
		int cat =0;
			...
			for (int j = 0; j<evt.nBToKEE;j++){
				...
				if (set==1){
					...
					cat1 = !evt.Electron_isPF[evt.BToKEE_l2Idx[j]];
				...
				bool vtxFitSel = evt.BToKEE_mass[j] <6 && evt.BToKEE_mass[j]>4.7 && evt.BToKEE_mll_fullfit[j]>0.045 && evt.BToKEE_mll_fullfit[j]<3.85/* && evt.BToKEE_svprob[j]>0.1 && evt. BToKEE_fit_cos2D[j]>0*/;
				...
				bool ele2Sel = ele2_convveto/* && evt.BToKEE_fit_l2_pt[j]>0.5 && fabs(evt.BToKEE_fit_l2_eta[j])44 && evt.Electron_isPF[evt.BToKEE_l2Idx[j]]*/;  
				bool kSel = evt.BToKEE_fit_k_pt[j]>0.8 && fabs(evt.BToKEE_fit_k_eta[j])<2.4; 


				if (/* vtxFitSel &&*/ ele1Sel && ele2Sel){ // decay products selection

					if (vtxFitSel){ // whole event selection


			if (fabs(evt.GenPart_pdgId[evt.Electron_genPartIdx[evt.BToKEE_l1Idx[j]]])==11 && fabs(evt.GenPart_pdgId[evt.Electron_genPartIdx[evt.BToKEE_l2Idx[j]]])==11 && fabs(evt.GenPart_pdgId[evt.ProbeTracks_genPartIdx[evt.BToKEE_kIdx[j]]])==321){
				if(/*evt.GenPart_pdgId[evt.GenPart_genPartIdxMother[evt.Electron_genPartIdx[evt.BToKEE_l1Idx[j]]]]==443 && evt.GenPart_pdgId[evt.GenPart_genPartIdxMother[evt.Electron_genPartIdx[evt.BToKEE_l2Idx[j]]]]==443 &&*/ (evt.GenPart_genPartIdxMother/*[evt.GenPart_genPartIdxMother*/[evt.Electron_genPartIdx[evt.BToKEE_l1Idx[j]]] ==  evt.GenPart_genPartIdxMother[evt.ProbeTracks_genPartIdx[evt.BToKEE_kIdx[j]]])  ){
						B_sel_disc2.push_back(evt.Electron_pt[evt.BToKEE_l1Idx[j]]+evt.Electron_pt[evt.BToKEE_l2Idx[j]]+evt.ProbeTracks_pt[evt.BToKEE_kIdx[j]]); // pushing back whichever variable to perform ranking of B candidates and choose the best candidate to be saved
						B_sel_disc1.push_back(evt.BToKEE_l_xy[j]/evt.BToKEE_l_xy_unc[j]); // pushing back whichever variable to perform ranking of B candidates and choose the best candidate to be saved
...
//				std::cout << " idx size 1 " << idx[1].size() << std::endl;
		if(idx[0].size()!=0){
			...
			else size =6; // if more than 3 reco B are found, saves the first three
	//		std::cout << "array size " << size << std::endl;
			...
			event = evt.event;
			prescale = 1;
			weight = 1./evt.BToKEE_fit_massErr[idx_check.at(k)];
			signal = 1;
